...
std::tuple<std::string, ListenOptions, EncapOptions> parse_interface_path(const std::string& path) {
    ...
    if ( bopen == 0 && bclose != path.npos ) {
        ...
        port_str = path.substr(pstart + 1, pend != path.npos ? (pend - pstart - 1) : path.npos);

        if ( pend != path.npos )
            opts_str = path.substr(pend + 1, path.npos);

        if ( inet_pton(AF_INET6, addr_str.c_str(), &lopts.addr.v6.sin6_addr) != 1 )
            return make_error(util::fmt("bad IPv6 listen address '%s'", addr_str.c_str()));
    }
    else {
        lopts.af = AF_INET;
        ...
        auto pend = path.find(':', pstart + 1);
        auto plen = pend == path.npos ? (path.size() - pstart - 1) : (pend - (pstart + 1));
        ...
        if ( inet_pton(AF_INET, addr_str.c_str(), &lopts.addr.v4.sin_addr) != 1 )
            return make_error(util::fmt("bad IPv4 listen address '%s'", addr_str.c_str()));
    }

    char* endp;
    lopts.port = strtol(port_str.c_str(), &endp, 10);
    if ( *endp || lopts.port <= 0 || lopts.port > std::numeric_limits<uint16_t>::max() )
    ...
    while ( pos < opts_str.size() ) {
        ...
        if ( opt_str == "raw" ) {
            if ( eopts.encap != Encapsulation::UNSET )
                return make_error(util::fmt("encapsulation already set - unexpected '%s'", opt_str.c_str()));
            ...
            if ( eopts.link_type >= 0 )
                return make_error(util::fmt("data link_type already set - unexpected '%s'", opt_str.c_str()));
...
void test_parse_interface_path() {
...

    {
        ...
        auto [error, lopts, eopts] = parse_interface_path("[::1]:4711:skip=13:dlt=ppp");
        myassert(error.empty(), "unexpected error");
        myassert(lopts.af == AF_INET6, "bad af");
        myassert(lopts.port == 4711, "wrong port");
        myassert(eopts.encap == Encapsulation::SKIP, "wrong encap");
...

    {
        ...
        myassert(error == "missing port in path '127.0.0.1'", "wrong error %s", error.c_str());
    }
...

    {
        ...
        myassert(! error.empty(), "no error for missing port");
        myassert(error == "missing port in path '[::1]'", "wrong error %s", error.c_str());
    }
...

    {
        ...
        myassert(error == "bad IPv4 listen address '192.168.300'", "wrong error %s", error.c_str());
    }

    {
        ...
        myassert(error == "bad IPv6 listen address 'scramble'", "wrong error %s", error.c_str());
    }
    ...
    {
        // Bad port 2**16
        auto [error, unused1, unused2] = parse_interface_path("[::1]:65536"); // one too large
    ...
    {
        // Double encap
        auto [error, unused1, unused2] = parse_interface_path("[::1]:4879:vxlan:geneve"); // one too large
        myassert(! error.empty(), "no error for vxlan:geneve");
        myassert(error == "encapsulation already set - unexpected 'geneve'", "wrong error %s", error.c_str());
    }

    {
        // Double skip
        auto [error, unused1, unused2] = parse_interface_path("[::1]:4879:skip=0:skip=4"); // one too large
        myassert(! error.empty(), "no error for skip=0:skip=4");
        myassert(error == "encapsulation already set - unexpected 'skip=4'", "wrong error %s", error.c_str());
    }

    {
        // Double link type
        auto [error, unused1, unused2] = parse_interface_path("[::1]:4879:dlt=raw:dlt=en10mb"); // one too large
        myassert(! error.empty(), "no error for DLT_RAW:DLT_EN10MB");
        myassert(error == "data link_type already set - unexpected 'dlt=en10mb'", "wrong error %s", error.c_str());
    }
