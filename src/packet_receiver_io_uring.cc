...
IOUringPacketReceiver::IOUringPacketReceiver(int fd, size_t sq_entries, size_t cq_entries, size_t buffers,
                                             size_t buf_shift)
    : fd(fd),
...
int add_recvmsg_multishot(struct io_uring* ring, struct msghdr* msg, int fdidx) {
...
bool IOUringPacketReceiver::Open() {
    ...
    UDPSOURCE_DEBUG("io_uring_queue_init_params() with sq=%zu cq=%zu", sq_entries, cq_entries);

    if ( io_uring_queue_init_params(sq_entries, &ring, &params) < 0 ) {
        fprintf(stderr, "io_uring_queue_init_params() failed: %s", strerror(errno));
        return false;
    }
    ...
    buffer_base = reinterpret_cast<std::byte*>(buf_ring) + sizeof(struct io_uring_buf) * buffers;
    ...
    for ( size_t i = 0; i < buffers; i++ ) {
        ...
        io_uring_buf_ring_add(buf_ring, buf, buf_size, i, io_uring_buf_ring_mask(buffers), i);
    }
...
bool IOUringPacketReceiver::NextPacket(RawPacket* pkt) {
    if ( n > 0 && i < n ) { // have some packets left in cqes. Fetch them.
        ...
        UDPSOURCE_DEBUG("cqe->res=%d cqe->flags=0x%02x bid=%d buf=%p out=%p", cqe->res, cqe->flags, bid, buf, out);

        // Parse out.
        if ( out ) {
            // Get timestamp into RawPacket.
            struct cmsghdr* cmsg = io_uring_recvmsg_cmsg_firsthdr(out, &msg);
            for ( ; cmsg != nullptr; cmsg = io_uring_recvmsg_cmsg_nexthdr(out, &msg, cmsg) ) {
            ...
            pkt->data = reinterpret_cast<const u_char*>(io_uring_recvmsg_payload(out, &msg));
...
void IOUringPacketReceiver::DoneWithPacket() {
    ...
    if ( cqe->flags & IORING_CQE_F_BUFFER ) {
        ...
        io_uring_buf_ring_add(buf_ring, buf, buf_size, bid, io_uring_buf_ring_mask(buffers), 0);
