...
RecvmmsgPacketReceiver::RecvmmsgPacketReceiver(int fd, size_t vlen, size_t iov_len) : fd(fd) {
...
bool RecvmmsgPacketReceiver::NextPacket(RawPacket* pkt) {
    if ( n > 0 && i < n ) {
        ...
        for ( struct cmsghdr* cmsg = CMSG_FIRSTHDR(msg_hdr); cmsg != nullptr; cmsg = CMSG_NXTHDR(msg_hdr, cmsg) ) {
    ...
    if ( r > 0 ) {
        ...
        if ( errno != EAGAIN && errno != EINTR ) {
            fprintf(stderr, "recvmmsg() failed: r=%d %s (%d), aborting...", r, strerror(errno), errno);
