...
void UDPSource::Open() {
    ...
    auto buf = static_cast<int>(zeek::BifConst::PacketSource::UDP::udp_recv_buffer_size);
    ...
    int timestampv = 1;
    if ( setsockopt(fd, SOL_SOCKET, SO_TIMESTAMP, &timestampv, sizeof(timestampv)) < 0 ) {
    ...
    if ( impl == recvmmsg_impl ) {
        ...
        size_t buffer_shift = zeek::BifConst::PacketSource::UDP::io_uring_buffer_shift;

        receiver = std::make_unique<IOUringPacketReceiver>(fd, sq_entries, cq_entries, buffers, buffer_shift);
#else
    ...
    if ( impl == recvmmsg_impl && zeek::BifConst::PacketSource::UDP::recvmmsg_use_selectable_fd ) {
    ...
    UDPSOURCE_DEBUG("Opened packet source with receiver implementation %s", obj_desc_short(impl).c_str());
    Opened(props);
}
...
void UDPSource::Close() {
    ...
    if ( fd >= 0 ) {
        if ( close(fd) != 0 )
            Error(util::fmt("failed to close socket fd=%d: %s (%d)", fd, strerror(errno), errno));
        fd = -1;
    }
...
#define NO_PACKET_DEBUG
    ...
    for ( int i = 0; i < rpkt.len && i < print_bytes; i++ ) {
        std::fprintf(stderr, "%02x%s", rpkt.data[i], i < (print_bytes - 1) ? ":" : "");
    }
    std::fprintf(stderr, " \n");
#endif
    ...
    switch ( encap_opts.encap ) {
        ...
        case Encapsulation::GENEVE: {
            ...
            if ( geneve_version != 0 ) {
                zeek::reporter->Weird("geneve_invalid_version", util::fmt("%d", geneve_version), "packet-source-udp");
            ...
            if ( pkt_data_caplen < (all_opt_len + 8) ) {
                zeek::reporter->Weird("geneve_too_short",
                                      util::fmt("pkt_data_caplen=%zu all_opt_len=%zu", pkt_data_caplen, all_opt_len),
                                      "packet-source-udp");
                goto skip_packet_no_weird;
            }
            ...
            if ( pkt_data_caplen < 20 ) {
                zeek::reporter->Weird("short_IP_in_GENEVE", util::fmt("%zu", pkt_data_caplen), "packet-source-udp");
                goto skip_packet_no_weird;
            }

            int ip_version = pkt_data[0] >> 4;
            if ( ip_version != 4 ) {
                zeek::reporter->Weird("unhandled IP version", util::fmt("%d", ip_version), "packet-source-udp");
                goto skip_packet_no_weird;
            }
            ...
            if ( proto != IPPROTO_UDP ) {
                zeek::reporter->Weird("non_UDP_proto", util::fmt("proto=%d", proto), "packet-source-udp");
                goto skip_packet_no_weird;
            }
        ...
        case Encapsulation::VXLAN: {
            ...
            if ( (pkt_data[0] & 0x08) == 0 ) {
                zeek::reporter->Weird("vxlan_unset_I_flag", util::fmt("%02x", pkt_data[0]), "packet-source-udp");
...
skip_packet:
    zeek::reporter->Weird("skipped_packet",
                          util::fmt("pkt_data_len=%zu encap=%d", pkt_data_len, static_cast<int>(encap_opts.encap)),
                          "packet-source-udp");
...
PktSrc* UDPSource::Instantiate(const std::string& path, bool is_live) {
    ...
    if ( ! error_msg.empty() ) {
        zeek::reporter->FatalError("packet-source-udp: invalid path: %s", error_msg.c_str());
        return nullptr;
    }
