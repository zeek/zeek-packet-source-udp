...
class GeneveVniConnKey : public zeek::IPBasedConnKey {
public:
    GeneveVniConnKey() {
...
protected:
    ...
    void DoPopulateConnIdVal(zeek::RecordVal& conn_id, zeek::RecordVal& ctx) override {
        ...
        ctx.Assign(GetVniOffset(), static_cast<zeek_uint_t>(key.geneve_vni));
    }

    void DoInit(const Packet& pkt) override { key.geneve_vni = UDPSource::GeneveVni(); }

    static int GetVniOffset() {
        static int geneve_vni_offset = zeek::id::conn_id_ctx->FieldOffset("geneve_vni");
        return geneve_vni_offset;
    }

private:
    friend class GeneveVniFactory;
...
zeek::ConnKeyPtr GeneveVniFactory::DoNewConnKey() const { return std::make_unique<GeneveVniConnKey>(); }

zeek::expected<zeek::ConnKeyPtr, std::string> GeneveVniFactory::DoConnKeyFromVal(const zeek::Val& v) const {
    ...
    int vni_offset = GeneveVniConnKey::GetVniOffset();
    static int ctx_offset = id::conn_id->FieldOffset("ctx");

    auto* k = static_cast<GeneveVniConnKey*>(ck.value().get());
    ...
    if ( ctx->HasField(vni_offset) )
        k->key.geneve_vni = ctx->GetFieldAs<zeek::CountVal>(vni_offset);
