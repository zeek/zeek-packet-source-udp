...
protected:
    ...
    void DoInit(const Packet& pkt) override { key.vxlan_vni = UDPSource::VxlanVni(); }

    static int GetVxlanVniOffset() {
        static int vxlan_vni_offset = zeek::id::conn_id_ctx->FieldOffset("vxlan_vni");
...
zeek::ConnKeyPtr VxlanVniFactory::DoNewConnKey() const { return std::make_unique<VxlanVniConnKey>(); }

zeek::expected<zeek::ConnKeyPtr, std::string> VxlanVniFactory::DoConnKeyFromVal(const zeek::Val& v) const {
