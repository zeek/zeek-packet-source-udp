...
protected:
    ...
    static int GetVxlanVniOffset() {
        static int geneve_vni_offset = zeek::id::conn_id_ctx->FieldOffset("vxlan_vni");
        return geneve_vni_offset;
    }
...
zeek::ConnKeyPtr GeneveVxlanVniFactory::DoNewConnKey() const { return std::make_unique<GeneveVxlanVniConnKey>(); }

zeek::expected<zeek::ConnKeyPtr, std::string> GeneveVxlanVniFactory::DoConnKeyFromVal(const zeek::Val& v) const {
